...
Если в директории присутствует `setup_split_routing.sh`, он будет вызван автоматически для настройки **разделённой маршрутизации на базе ipset**:
- Трафик в РФ (по списку IP) пойдёт напрямую через WAN.
- Остальной трафик пойдёт через туннель (`tun0`).
- Настраиваются systemd-сервисы для автозагрузки правил и ежедневного обновления списков РФ.
